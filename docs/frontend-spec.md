...
**Connection health badge** (`components/shared/connection-health-badge.tsx`) — a per-connection chip driven by the backend-derived `health` field on `GET /connections` (`healthy` / `expiring` / `expired` / `error`, `frontend/src/lib/api/connections.ts`). It renders **nothing** when `healthy` so a well-connected platform shows no warning (P-2 — no permanently-lit badge), an amber **"Token expires <relative>"** when `expiring`, a red **"Token expired — reconnect"** when `expired`, and a red **"Connection error"** with `last_error` / `last_error_at` in a tooltip when `error`. The connections query refetches on a 60s interval + on window focus, so the badge lights up when a token lapses and clears itself the moment the backend reports healthy again (P-2). A **Reconnect** button surfaces on the card only while `expired` or `error` (OAuth re-initiate for TikTok/Google, the token dialog for Meta) — the recovery path for the cases proactive token refresh can't self-heal (P-5); it is not shown for healthy/expiring connections.
...
### `NotificationCenter`
`components/shared/notification-center.tsx` — the bell + unread-count badge + inbox Popover in the top chrome, rendered in both the dashboard `TopBar` and the settings `Header` (`hidden ... sm:flex`). Backed by `useNotifications()` (`frontend/src/hooks/use-notifications.ts`) → `GET /notifications?status=unread` (`frontend/src/lib/api/notifications.ts`) via TanStack Query with `refetchInterval` **45s** (`NOTIFICATIONS_POLL_MS`) + `refetchOnWindowFocus` — **polling, not websockets**, since worker-generated alerts are minute-scale. The red count badge is hidden entirely when `unread_count === 0`, and because a resolved notification drops out of the `unread` list on the next poll, the badge clears itself automatically (P-2 — no permanently-lit badge). The Popover lists unread items (severity icon: info/warning/error, title, 2-line body, relative time); clicking a row calls `markRead(id)` (optimistically removing it and decrementing the count via `useMarkNotificationRead`) and, when the notification carries a stored `deep_link`, navigates there — the link is a stored column, never inferred from message text (anti-req §2.3). Empty state reads "You're all caught up." Replaces the earlier inert placeholder bell button in `TopBar`.
